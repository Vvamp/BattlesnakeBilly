...
public class Tree{
    ...
    private Node root;
    public Node findSpotToExpand(Node root){
        ...
        if(validMoves.isEmpty()) {
            ...
            return findSpotToExpand(newRoot);
        }else{
    ...
    public Node expand(Node root){
        ...
        GameState newGameState = new GameState(root.getGamestate()); //deep copy old gamestae
    ...
    public double rollout(Node leaf){
        GameState currentState = new GameState(leaf.getGamestate()); //deep copy gs
        ...
        while(currentState.isTerminal() == false){
            ...
            for(Battlesnake snake : players){
                ...
                Move newMove = legalMoves.get(new Random().nextInt(legalMoves.size()));        // TODO: improvements here are possible
                moves.put(snake, newMove);
            }

            currentState = new GameState(currentState); //deep copy old gamestate
            for (Map.Entry<Battlesnake, Move> entry : moves.entrySet()) {
                currentState.applyMove(entry.getKey(), entry.getValue()); // Apply moves for each snake
            }
        }
